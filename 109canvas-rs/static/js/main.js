...
function init(mod) {
    if (typeof mod === 'undefined') {
        mod = import.meta.url.replace(/\.js$/, '_bg.wasm');
        console.console.log(mod);
    }
    let result;
    const imports = {};
    imports.wbg = {};
    ...
    if ((typeof URL === 'function' && mod instanceof URL) || typeof mod === 'string' || (typeof Request === 'function' && mod instanceof Request)) {

        const response = fetch(mod);
        ...
        result = WebAssembly.instantiate(mod, imports)
            .then(result => {
                if (result instanceof WebAssembly.Instance) {
                    return { instance: result, mod };
                } else {
                    return result;
                }
            });
    }
    return result.then(({ instance, mod }) => {
        wasm = instance.exports;
        init.__wbindgen_wasm_module = mod;
